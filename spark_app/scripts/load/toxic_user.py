...
def get_batch_user_toxic(batch_df: pd.DataFrame):
    ...
    return (
            ...
            .agg(
                ...
                F.first("author_name").alias("author_name"),
                F.first("author_image").alias("author_image"),
                
                F.count("comment_id").alias("total_comments"),
                F.sum((F.col("toxic_score") >= 0.7).cast("long")).alias("toxic_count"),
                F.max(
                    F.when(F.col("toxic_score") >= 0.7, F.col("published_at"))
                ).alias("last_violation_time")
            )
            .filter(F.col("toxic_count") > 0)
            .withColumn("toxic_ratio", F.col("toxic_count") / F.col("total_comments"))
            .select(
                "video_id", "author_id", "author_name", "author_image",
                "total_comments", "toxic_count", "last_violation_time", "toxic_ratio",
...
def write_to_mongo_user_toxic_batch(batch_df, batch_id):
    ...
    # exactly-once
    if db.processed_batches.find_one({"batch_id": batch_id,"table_name":"toxic_user_metric"}):
        return
